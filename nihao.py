...
class MixAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, dwconv_kernel_size, num_heads, qkv_bias=True, qk_scale=None,
        ...
        self.qkvc = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=False)
        self.qkv_dwconv = nn.Conv2d(dim * 3, dim * 3, kernel_size=3, stride=1, padding=1, groups=dim * 3, bias=False)
        self.project_out = nn.Conv2d(dim, dim, kernel_size=1, bias=False)
        # self.dwconv3x3 = cAttention(self.dim, self.num_heads, False)
        #### Spectral branch
        ...
        self.spatial_interaction = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(dim // 16, dim, kernel_size=1)  # 最终空间信息输出通道为1
        )
    ...
    def forward(self, x, H, W, mask=None):
        ...
        qkvc = self.qkv_dwconv(self.qkvc(x_cnn))
        qc, kc, vc = qkvc.chunk(3, dim=1)

        qc = rearrange(qc, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        kc = rearrange(kc, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        vc = rearrange(vc, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        spatial_interaction = torch.sigmoid(spatial_interaction)
        spatial_interaction = rearrange(spatial_interaction, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        vc = spatial_interaction * vc

        qc = torch.nn.functional.normalize(qc, dim=-1)
        kc = torch.nn.functional.normalize(kc, dim=-1)

        attnc = (qc @ kc.transpose(-2, -1)) * self.temperature
        ...
        out = self.project_out(out)
        channel_interaction = self.channel_interaction(out)
        ...
        # concat
        x_atten = x_spatial * channel_interaction
        # cat(): -> [num_windows*B, N, C] --- C: C
        x = torch.cat([out, x_atten], dim=1)
