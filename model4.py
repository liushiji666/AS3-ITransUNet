...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        # scale = 2
        self.body1 = nn.Sequential(nn.Conv2d(n_feat, n_feat * 4, kernel_size=3, stride=1, padding=1, bias=False),
                                  nn.PixelShuffle(2))
        self.body2 = nn.Sequential(nn.Conv2d(n_feat, n_feat * 2, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.PixelShuffle(2))
        # modules = []
        # for _ in range(int(math.log(scale, 2))):
        #     modules.append(nn.Conv2d(n_feat, n_feat * 2, kernel_size=3, stride=1, padding=1, bias=False))
...
class Restormer(nn.Module):
    def __init__(self,
        ...
        self.reduce_chan_level1i = nn.Conv2d(int(dim * 2), int(dim * 2 ** 1), kernel_size=3, stride=1, padding=1,
                                             bias=bias)
        ...
        self.reduce_chan_level3i = nn.Conv2d(int(dim * 2), int(dim * 2 ** 1), kernel_size=3, stride=1, padding=1,
                                             bias=bias)
